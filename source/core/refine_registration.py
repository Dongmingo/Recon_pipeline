"""
 Refine registration: the rough alignments are aligned more tightly.
...
from utils.utility import draw_registration_result_original_color, matching_result
...
def optimize_posegraph_for_scene(data_dict, config):
    ...
    pose_graph_name = data_dict['template_registered_posegraph']
    pose_graph_optimized_name = data_dict['template_refined_posegraph']
    run_posegraph_optimization(pose_graph_name, pose_graph_optimized_name, opt_option)
    
    
def make_posegraph_for_refined_scene(data_dict, config):
    ...
    if config["python_multi_threading"] == True:
        ...
        results = Parallel(n_jobs=MAX_THREAD)(
            delayed(register_point_cloud_pair)(
                data_dict, matching_results[r].s, matching_results[r].t,
    ...
    o3d.io.write_pose_graph(data_dict['template_refined_posegraph'],pose_graph_new)